...
int parse_cmd(int argc, char *argv[], parse_info_t *info) {
    ...
    for (int i = 0; i < argc; i++) {
        if (strcmp(argv[i], "-t") == 0) {
            ...
            if (i + 1 >= argc || strlen(argv[i + 1]) == 0 || str_isDigit(argv[i + 1]) < 1) {
                write(STDERR_FILENO, "Flag -l missing an integer\n", 27);
                flags |= FLAG_ERR;
                return flags;
            }

            sscanf(argv[i + 1], "%d", &(info->capacity));
            flags |= FLAG_PLACES;  // capacity flag
            i++;
        } else if (strcmp(argv[i], "-n") == 0) {
            if (i + 1 >= argc || strlen(argv[i + 1]) == 0 || str_isDigit(argv[i + 1]) < 1) {
                write(STDERR_FILENO, "Flag -n missing an integer\n", 27);
                flags |= FLAG_ERR;
                return flags;
            }

            sscanf(argv[i + 1], "%d", &(info->max_threads));
            flags |= FLAG_THREADS;  // maximum number of threads flag
            i++;
        } else {

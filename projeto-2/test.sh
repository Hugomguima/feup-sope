...
test_logs() {
  ...
  if [ "$nIAMIN" != "$nENTER" ] || [ "$nENTER" != "$nTIMUP" ]; then
    fail=1
    echo "nIAMIN != nENTER or nENTER != nTIMUP"
  fi
  ...
  [ -z "$(cat "$q_log")" ] || [ -z "$(cat "$u_log")" ] && echo "WARNING. At least 1 empry log file."
